...
chaincodeInvoke() {
    ...
    ## Add private data
    export HUMAN=$(echo -n "{\"key\":\"1111\", \ \"Latitude\":\"37\",\"Longitude\":\"126\",\"Timestamp\":\"1637938956368\",\"Barometer\":\"102656\",\"Heartrate\":\"91\",\"Name\":\"Hakpyeong Kim\"}" | base64 | tr -d \\n)
    ...
    peer chaincode invoke -o localhost:7050 \
        ...
        -C $CHANNEL_NAME -n ${CC_NAME} \
        --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_ORG1_CA \
        -c "{\"function\":\"createHumanCentered\",\"Args\":\"$HUMAN\"}"
        # --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_ORG2_CA \
}
...
chaincodeQuery() {
    ...
    # Query Car by Id
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "queryHumanCentered","Args":["HUMAN0]}'
    # peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "queryHumanCentered","Args":["1111]}'
